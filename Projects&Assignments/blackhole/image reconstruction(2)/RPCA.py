...
img_obj = Image.open("timg.jpg")
#img_obj.show()
groundtruth = np.array(img_obj, dtype=np.uint8)
...
#使图片部分像素不可见/损毁
percentage = 0.75  #不可见比例
imageheight = np.size(groundtruth, 0) #每列元素个数
imagewidth = np.size(groundtruth, 1)  #每行元素个数
